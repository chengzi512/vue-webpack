...
function responseInterceptor(res){
  if (res.status === 200) {
    ...
    if(res.data.code != 0){
      if(res.data.code === 1){
        setTimeout(function () {
          window.location.href = isDevMode ? '/login.html' + '?callback=' + encodeURI(location.href) : '/login/index.html' + '?callback=' + encodeURI(location.href)
        },3000)
      }
      console.log(res.data.result, res.request.responseURL);
      msgHub.$emit('api-error-info', res.data);
    }
    return res.data;
  } else {
    console.log(res)
